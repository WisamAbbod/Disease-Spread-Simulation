...
public class InitPeople : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        int TotalBlobs = SceneVars.TotalBlobs;
        GameObject PrefabChoice;
            ...
            for (int i = 0; i < TotalBlobs; i++)
            {
                ...
                if (Random.Range(0, 100) < sv.PCInfected)
                {
                    ...
                    sv.Cells[i].GetComponent<Movement>().TickRecovered = sv.CurrentTick + Random.Range(1, (SceneVars.RecoveryPeriod + 1)); //pre existing infection

                    sv.TotalInfected++;
                    sv.TotalNeverInfected--;
                }

                if (Random.Range(0, 100) < SceneVars.PCMasks)
                {
