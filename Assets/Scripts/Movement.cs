...
public class Movement : MonoBehaviour
{
    ...
    public Rigidbody rb;                         //setting all the variables
    ...
    {
        SceneVars sv = go.GetComponent<SceneVars>();    //calls class for probabilities
        ChangeDirection = true;
        if (IsDead == false && other.GetComponent<Movement>().Infected == true && Infected == false && Immune == false) // set infected
        {
            int OurInfectLevel = SceneVars.InfectionProb;                                      
            int OtherInfectLevel =SceneVars.InfectionProb;
    ...
    void Start()  //starting the the cells moving around
    {
    ...
    void Update()
    {
        ...
        if (Infected == true && sv.CurrentTick > TickRecovered) //recovered - set immune
        {
            ...
            sv.TotalInfected--;
            sv.TotalImmune++;
            ImmuneExpires = sv.CurrentTick + SceneVars.ImmunityPeriod;
    ...
    void BecomeInfected()
    {
        ...
        sv.TotalInfected++;
        TotalTimesInfected++;
        TickRecovered = sv.CurrentTick + SceneVars.RecoveryPeriod;
